...
<{        if $smarty.foreach.sublp.first}>    <ul class="dropdown-menu"><!-- sub menu --><{/if}>
<{        if ($xlanguage && (($menu.title|strstr:$myStr) OR ($menu.image|strstr:$myStr)) OR !$xlanguage)}>
<{          if $menu.id == $sub.pid}>
      <li<{if $sub.css}> class="<{$sub.css}>"<{/if}>>
        <a<{if $sub.selected}> class="active"<{/if}> href="<{$sub.link}>" target="<{$sub.target}>" title="<{$sub.alt_title}>"><{$sub.title}></a>
<{            if !empty($block.subsub)}>
<{              foreach item=subsub from=$block name=subsublp}>
<{                if $smarty.foreach.subsublp.first}>        <ul class="dropdown-menu"><!-- subsub menu --><{/if}>
<{                if $sub.id == $subsub.pid}>
          <li<{if $subsub.css}> class="<{$subsub.css}>"<{/if}>>
            <a<{if $subsub.selected}> class="active"<{/if}> href="<{$subsub.link}>" target="<{$subsub.target}>"  title="<{$subsub.alt_title}>"><{$subsub.title}></a>
<{                /if}>
<{                if $smarty.foreach.subsublp.last}></ul><{/if}>
<{              /foreach}>
<{            /if}>
<{          /if}>
<{        /if}>
