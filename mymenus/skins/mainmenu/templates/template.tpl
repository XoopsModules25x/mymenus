...
<{    if ($xlanguage && (($menu.title|strstr:$myStr) OR ($menu.image|strstr:$myStr)) OR !$xlanguage)}>
    <a class="menuMain<{if $menu.css}> <{$menu.css}><{/if}><{if $menu.topselected}> maincurrent<{/if}>" href="<{$menu.link}>" target="<{$menu.target}>" title="<{$menu.alt_title}>">
    <{if $menu.image}><img src="<{$menu.image}>"/><{/if}><{$menu.title}></a>
    <br />
<{      if $menu.selected}>
