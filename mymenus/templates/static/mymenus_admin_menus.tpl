...
<{else}>
    <table class='outer width100' cellspacing='1'>
        <tr class='odd'>
            ...
            <form id='filter_form' name='filter_form' action='' method='post' enctype='multipart/form-data'>
                ...
                <input id='filter_menus_title' type='text' value='<{$filter_menus_title}>' maxlength='255' size='25' title='' name='filter_menus_title'>
                &nbsp;
                <input type='submit' id='filter_submit' class='formButton' title='<{$smarty.const._AM_MYMENUS_BUTTON_FILTER}>' value='<{$smarty.const._AM_MYMENUS_BUTTON_FILTER}>' name='filter_submit'>
    ...
    <{foreach item=menu key=key from=$menus}>
        <tr class="<{cycle values="even,odd"}>" align="center">
            ...
            <td>
                <a href="?op=edit&amp;id=<{$menu.id}>"><img src="<{xoModuleIcons16 edit.png}>" title="<{$smarty.const._EDIT}>" alt="<{$smarty.const._EDIT}>" /></a>
                <a href="?op=delete&amp;id=<{$menu.id}>"><img src="<{xoModuleIcons16 delete.png}>" title="<{$smarty.const._DELETE}>" alt="<{$smarty.const._DELETE}>" /></a>
                <a href="links.php?op=list&amp;mid=<{$menu.id}>"><img src="<{xoModuleIcons16 forward.png}>" title="<{$smarty.const._AM_MYMENUS_ACTION_GOTO_MENU}>" alt="<{$smarty.const._AM_MYMENUS_ACTION_GOTO_MENU}>" /></a>
            </td>
        </tr>
    <{/foreach}>
    </table>
